...
def main():
    ...
    parser.add_argument("--dataset_path", default=str(os.getenv("DATASET_PATH_CWOLA")), type=str, help="Path to the LHCO dataset")
    ...
    parser.add_argument("--n_jets_train", type=list, default=list(map(int,os.getenv("N_JETS_TRAIN_CWOLA_AACHEN").strip('[]').split(','))), help="Number of jets per class for training [signal, supp, background]")
    ...
    signal_path = os.path.join(args.dataset_path, "signal_SR.h5")
    supp_background_path = os.path.join(args.dataset_path, "background_SR_supp.h5")
    supp_signal_path = os.path.join(args.dataset_path, "signal_SB_supp.h5")
    background_path = os.path.join(args.dataset_path, "background_SB.h5")
    
    h5_files_all = [signal_path, supp_background_path, supp_signal_path, background_path]
    ...
    data_config = {
        ...
        "supp_signal_file": supp_signal_path,
    ...
    train_loader, val_loader = create_custom_lhco_h5_dataloaders(
        ...
        batch_size=args.batch_size,
        n_jets_train=args.n_jets_train,  # [signal, supplementary_background, supplementary_signal, background]
        max_sequence_len=128,
        mom4_format="epxpypz",
        jet_name=args.jet_name,
        train_val_split=args.train_val_split,
    ...
    if args.use_class_weights:
        ...
        n_label_0 = args.n_jets_train[2] + args.n_jets_train[3]  # supp signal + background

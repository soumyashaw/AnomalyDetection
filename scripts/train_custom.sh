...
apptainer exec --nv "$IMG" bash -lc '
    ...
    python -m src.train.train_custom --gpu_id 0 --seed 42 --jet_name both --merge_strategy concat --naming_identifier weak_5k_weighted_sum --use_wandb 2>&1 | tee "$TRAIN_LOG"
    ...
    python -m src.eval.evaluate --checkpoint "$BEST_CKPT" --model_type dijet
'

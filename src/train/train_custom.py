""" Python file to train the OmniJet-alpha model with continuous tokens - Custom weak supervision variant.

PROJECT STRUCTURE:
AnomalyDetection/
├── src/
│   ├── train/              # Training scripts (e.g., train.py, train_custom.py)
...
└── Training
    ...
    ├── AdamW + Scheduler
    ├── Metric callbacks (AUC)
    └── Weights & Biases logging (optional)

USAGE: python -m src.train.train_custom [args]
Training a custom weak supervision model on LHCO datasets. (Trained on 200k bkg + 100k polluted + 10k-25k signal jets) (Tested on 200k bkg + 50k signal jets)
"""
# imports
import os
import json
...
def main():
    ...
    parser.add_argument("--n_jets_train", type=list, default=list(map(int,os.getenv("N_JETS_TRAIN_CUSTOM").strip('[]').split(','))), help="Number of jets per class for training [signal, supp, background]")
    ...
    parser.add_argument("--log_dir", type=str, default=str(os.getenv("LOG_DIR_CUSTOM")), help="Directory for experiment logs")

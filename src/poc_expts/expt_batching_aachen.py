    ...
    def create_guaranteed_signal_loader(signal_dict, supp_bg_dict, clean_bg_dict, 
        ...
        class BatchCollator:
            ...
            def __init__(self, signal_dict, supp_bg_dict, clean_bg_dict, 
                        batch_size, guaranteed_signal, do_shuffle):
                ...
                n_signal = len(signal_dict["part_features"])
            ...
            def __call__(self, batch_indices):
                ...
                # Construct batch: [clean_bg (label 0), signal (label 1), supp_bg (label 1)]
                batch_data = {}
                for key in self.clean_bg_dict:
                    parts = [
                        self.clean_bg_dict[key][clean_start:clean_end],
                        self.signal_dict[key][signal_start:signal_end],
                        self.supp_bg_dict[key][supp_start:supp_end],
                    ]
                    batch_data[key] = torch.cat(parts, dim=0)
...
def main():
    ...
    parser.add_argument("--guaranteed_signal_per_batch", type=int, default=0, 
                       help="[DIAGNOSTIC] Number of guaranteed true signal jets per batch (k). Set to 0 to use standard weak supervision. Set to 1-4 for ablation studies.")
    ...
    if args.guaranteed_signal_per_batch > 0:
        ...
        print(f"\n{'='*80}")
        print(f"DIAGNOSTIC ABLATION STUDY: Guaranteed Signal Injection")
        print(f"{'='*80}")

...
│   ├── viz/                # Visualization (e.g., visualize_clustering.py)
│   └── poc_expts/          # Proof-of-concept experiments
├── scripts/                # Job launchers
├── gabbro/                 # Core library (data loaders, preprocessing)
└── [output directories: plots/]

FEATURES:
├── Load signal and background data with same preprocessing as train.py
├── Apply t-SNE for nonlinear dimensionality reduction
├── Apply UMAP for faster alternative
├── Apply PCA for linear baseline
├── Compute silhouette scores
├── Visualize with seaborn heatmaps
└── Pre-training analysis before model training

USAGE: python -m src.viz.visualize_clustering [args]
Loads signal and background jet data with the same preprocessing pipeline as train.py
and applies multiple dimensionality reduction techniques to check for emergent clustering.
"""
